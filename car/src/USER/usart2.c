...
static uint8_t u8Usart2_Halffull = 1;					// 0代表是前半个缓冲区满了，1代表是后半个缓冲区满了
...
void DMA1_Channel6_IRQ(void)
{
		uint32_t i;
 		...
 		if (DMA1->ISR & DMA1_IT_HT6)										// 通道6半传输事件完成
 		{ 

			 DMA1->IFCR |= DMA1_FLAG_HT6;									// 清除通道6半传输完成标志
			 u8Usart2_Halffull = 0;												// 前半个缓冲区满
			 ...
			 for (i = 0; i < (USART2_DMA_RX_BUFF_SIZE_HALF); i++)
			 {
					 ...
					 sUsart2_entity.u16RxCounter++;						//数组指针指向下一个单元
		...
		else if (DMA1->ISR & DMA1_IT_TC6)								// 通道6传输事件完成
 		{ 
						u8Usart2_Halffull = 1;									// 后半个缓冲区满
 						DMA1->IFCR |= DMA1_FLAG_TC6;						// 清除通道6传输完成标志
...
void USART2_IRQ(void)
{
	...
	if (USART2->SR & USART_FLAG_IDLE)									//如果检测到空闲位
	{
			u32Temp = USART2->SR;  
      u32Temp = USART2->DR; 												//清USART_IT_IDLE标志   

			if (u8Usart2_Halffull == 0)										//如果前半个缓冲区满
			{
				 dmabuflen = (USART2_DMA_RX_BUFF_SIZE_HALF) - DMA1_Channel6->CNDTR;			//取得现在一共传输了多少个数据
			}
			else if (u8Usart2_Halffull == 1)							//后半个缓冲区满
			{
			...
			{
					if (u8Usart2_Halffull == 0)								//前半个缓冲区满
					{
						 u8Usart2_User_RX_Buffer[sUsart2_entity.u16RxCounter] = sUsart2_entity.u8RX_Buffer[USART2_DMA_RX_BUFF_SIZE_HALF + i];
						
						 sUsart2_entity.u16RxCounter++;					//数组指针指向下一个单元
					}
					else if (u8Usart2_Halffull == 1)					//如果后半个缓冲区满
					{
							 u8Usart2_User_RX_Buffer[sUsart2_entity.u16RxCounter] = sUsart2_entity.u8RX_Buffer[i];
			...
			sUsart2_entity.u16RxTotalNum += dmabuflen;			//如果空闲，加上多余的数据个数
...
void Usart2_Send(uint8_t *u8Buf, uint32_t u32Len)
{
	...
	if (u32Tail == u32Head)
	{
			...
			for (i = 0; i < u32Len; i++)
			{
					RingBufWriteOne(&sUsart2_RingBuf, u8Buf[i]);					// 将需要的需要的数据量存入环形队列
